...
public:
    ...
    void Init()
    {
		...
		HBKINFOM hBkInfoM = XEle_GetBkInfoManager(hSbar); //获取滚动条的背景管理器
		HIMAGE hImageBG = XImage_LoadFile(L"bgV.png",TRUE);
		XBkInfoM_AddImage(hBkInfoM,element_state_flag_leave,hImageBG); //然后贴图
		
		
		HELE hBtnUp = XSBar_GetButtonUp(hSbar); //获取按钮
		XBtn_SetStyle(hBtnUp,button_style_default);
		XEle_EnableBkTransparent(hBtnUp,TRUE);
		hBkInfoM = XEle_GetBkInfoManager(hBtnUp); //获取按钮的背景管理器
		HIMAGE hImageUp = XImage_LoadFile(L"up.png",FALSE);
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_leave,hImageUp); //然后贴图
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_stay,hImageUp); //然后贴图
 		
		HELE hBtnDown = XSBar_GetButtonDown(hSbar);
		XBtn_SetStyle(hBtnDown,button_style_default);
		XEle_EnableBkTransparent(hBtnDown,TRUE);
		hBkInfoM = XEle_GetBkInfoManager(hBtnDown); //获取按钮的背景管理器
		HIMAGE hImageDown = XImage_LoadFile(L"up.png",FALSE);
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_leave,hImageDown); //然后贴图
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_stay,hImageDown); //然后贴图
 		

		HELE hSlider = XSBar_GetButtonSlider(hSbar);
		XEle_EnableBkTransparent(hSlider,TRUE);
		hBkInfoM = XEle_GetBkInfoManager(hSlider); //获取按钮的背景管理器
		HIMAGE hImageSlider = XImage_LoadFile(L"sliderV.png",TRUE);
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_leave,hImageSlider); //然后贴图
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_stay,hImageSlider); //然后贴图
		XBkInfoM_AddImage(hBkInfoM,button_state_flag_down,hImageSlider); //然后贴图
