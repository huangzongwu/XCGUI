...
public:
	...
	{
		m_hEdit = hRichEdit;
		HWND hWnd = XEle_GetHWND(hRichEdit);
		m_hDC = GetDC(hWnd);

		HFONTX hFontX = XEle_GetFont(hRichEdit);

		HFONT hFont = XFont_GetHFONT(hFontX);

		HFONT hOrgFont = (HFONT)::SelectObject(m_hDC,hFont);
		...
		SIZE TextSize;
		GetTextExtentPoint32W(m_hDC,pBuffer,nLen,&TextSize);
